...
日志格式是 JSONL：每条日志是一行完整 JSON，默认输出到 stdout/stderr，便于 Docker、systemd 或日志采集系统按行解析。项目不会默认把服务端运行日志写进 SQLite；SQLite 中保存的是业务数据、AI 分析历史和 AI 聊天会话。

如需同时落盘为 JSONL 文件，可设置 `APP_LOG_FILE`：

```bash
APP_LOG_FILE=data/app.jsonl pnpm dev
```

每条记录包含 `ts`、`level`、`event` 和调用方传入的字段。敏感字段名（例如 `apiKey`、`authorization`、`token`、`password`）会被替换为 `[redacted]`。
