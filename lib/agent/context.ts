...
function buildAgentSystemPrompt(language: AiConfig['analysisLanguage']) {
  return [
    ...
    'skillResults、answerDraft.source、requiredSkills、skillName 以及形如 portfolio.getSummary、stock.getHolding、web.search 的名称都是系统内部标识，只能用于你理解证据来源，严禁在面向用户的最终回复中原样提及。',
    '最终回复必须使用自然语言描述数据来源，例如“根据你的当前持仓数据”“从最近交易记录看”“公开搜索结果显示”，不要写“基于某某 Skill / getSummary / getTopPositions 的数据”。',
    ...
    '当 skillResults 包含 trade.prepareRecord 且 status 为 pending_confirmation 时，你只能返回待确认草稿并请用户核对确认，不得声称已经写入数据库。',
    '当 skillResults 包含 trade.prepareRecord 且 status 为 needs_more_info 时，你需要列出缺失字段并继续追问；当包含 trade.commitRecord 且 status 为 recorded 时，才可以说明已录入。',
...
function buildContextPrompt(contextSnapshot: Record<string, unknown>) {
  return [
    ...
    '如果某个数据读取步骤执行失败或返回空数据，请说明该数据不足，而不是猜测。',
    'answerDraft 是优先回答依据；skillResults 是原始证据。若两者冲突，以 skillResults 为准并说明不确定性。',
    '注意：JSON 中的 skillName、source、requiredSkills 等字段是内部审计标识，不是用户可见文案；最终回复不得出现这些内部标识或“Skill/技能/getSummary/getTopPositions”等说法。',
